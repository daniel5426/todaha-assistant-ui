...
const Chatbot = () => {
  ...
  const [chatbotConfig, setChatbotConfig] = useState<IChatBot | null>(null);
  ...
  useEffect(() => {
    const fetchInitialMessage = async () => {
      try {
        const response = await axios.get(`${api_url}/chat/get-inital-message`, {
          params: { assistant_id: assistantId },
        });
        setInitialMessage(response.data.welcome_msg);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchChatbotConfig = async () => {
      try {
        const response = await axios.get(`${api_url}/chat/get-chatbot`, {
          params: { assistant_id: assistantId },
        });
        setChatbotConfig(response.data.chatbot as IChatBot);
      } catch (error) {
  ...
  const type_placeholder = isRTL ? "כתוב כאן..." : "Write here...";
  const active_text = isRTL ? "פעיל" : "Active";
                    ...
                    >
                      <span className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-[12px]">{active_text}</p>
                        </div>
                      </span>
                    </div>
                  </div>
